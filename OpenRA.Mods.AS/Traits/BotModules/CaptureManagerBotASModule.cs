...
namespace OpenRA.Mods.AS.Traits
{
	...
	public class CaptureManagerBotASModuleInfo : ConditionalTraitInfo
	{
		...
		public readonly PlayerRelationship CapturablePlayerRelationships = PlayerRelationship.Enemy | PlayerRelationship.Neutral;
		...
		void QueueCaptureOrders(IBot bot)
		{
			...
			if (world.LocalRandom.Next(100) < Info.PriorityCaptureChance)
			{
				var priorityTargets = world.Actors.Where(a =>
					!a.IsDead && a.IsInWorld && Info.CapturablePlayerRelationships.HasRelationship(player.RelationshipWith(a.Owner))
			...
			var randPlayer = world.Players.Where(p => !p.Spectating
				&& Info.CapturablePlayerRelationships.HasRelationship(player.RelationshipWith(p))).Random(world.LocalRandom);
