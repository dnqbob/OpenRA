...
namespace OpenRA.Mods.AS.Traits
{
	...
	class GivesProximityBountyInfo : ConditionalTraitInfo
	{
		...
		[Desc("PlayerRelationship the attacking player needs to grant bounty to actors.")]
	...
	class GivesProximityBounty : ConditionalTrait<GivesProximityBountyInfo>, INotifyKilled
	{
		...
		void INotifyKilled.Killed(Actor self, AttackInfo e)
		{
			...
			foreach (var c in Collectors)
			{
				...
				if (!c.Info.ValidStances.HasRelationship(e.Attacker.Owner.RelationshipWith(self.Owner)))
					return;
