...
{
	[Desc("Applies a condition to actors within a specified range around the ground position.")]
	public class ProximityExternalConditionASInfo : ConditionalTraitInfo
	{
		[FieldLoader.Require]
		...
		public override object Create(ActorInitializer init) { return new ProximityExternalConditionAS(init.Self, this); }
	}

	public class ProximityExternalConditionAS : ConditionalTrait<ProximityExternalConditionASInfo>, ITick, INotifyAddedToWorld, INotifyRemovedFromWorld, INotifyOtherProduction
	{
		readonly Actor self;
		...
		public ProximityExternalConditionAS(Actor self, ProximityExternalConditionASInfo info)
			: base(info)
		{
			this.self = self;
		...
		{
			cachedPosition = self.CenterPosition - new WVec(WDist.Zero, WDist.Zero, self.World.Map.DistanceAboveTerrain(self.CenterPosition));
