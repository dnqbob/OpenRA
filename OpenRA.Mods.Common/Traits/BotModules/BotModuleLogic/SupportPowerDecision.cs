...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class SupportPowerDecision
	{
		...
		public readonly Dictionary<string, SupportPowerStrategy> Considerations = new();
		...
		[Desc("When set to true, the bot will find target location earlier than extra location, and vice versa",
			"when earlier check is ActorLocation or ActorCenter and later check is CellCanBeEnteredByTargetedActor," +
			" the targetted actor will be passed from early check to later check.")]
		...
		public bool IsHarmfulLocationConsideration(string strategyName, bool asExtraPos)
		{
			return strategyName != null && Considerations.TryGetValue(strategyName, out var strategy) &&
				((asExtraPos && strategy.ExtraPositionConsiderations.Any(c => c.TargetMetric == Consideration.DecisionMetric.HarmfulCell)) ||
				(!asExtraPos && strategy.TargetPositionConsiderations.Any(c => c.TargetMetric == Consideration.DecisionMetric.HarmfulCell)));
		}
		...
		{
			public string Name;
			public List<Consideration> TargetPositionConsiderations = new();
			public List<Consideration> ExtraPositionConsiderations = new();
		}

		/// <summary>Makes up part of a decision, describing how to evaluate a target.</summary>
		public class Consideration
		{
			public enum DecisionMetric { Health, HealthLoss, Value, None, HarmfulCell, Undetected }
			...
			public readonly HashSet<string> ValidActorTypes = new();

			[Desc("What types of actor should the undesired targets of this power be?")]
			public readonly HashSet<string> InvalidActorTypes = new();
